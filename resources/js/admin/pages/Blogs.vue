					...
					<div class="_overflow _table_div">
						<table class="_table">
							...
							<tr v-for="(blog, i) in blogs" :key="i" v-if="blogs.length">
								...
								<td> 
                                    <span  v-for="(category, j) in blog.categories" v-if="blog.categories.length">
                                        <Tag type="border">{{category.categoryName}}</Tag>
                                    </span> 
                                </td>
								<td> 
                                    <span v-for="(tag, j) in blog.tags" v-if="blog.tags.length">
                                        <Tag type="border">{{tag.tagName}}</Tag>
                                    </span> 
                                </td>
								<td> {{blog.views}}</td>
                                <td>
...
export default {
	...
	methods : {
		...
		async getBlogData(page = 1) {
			const res = await this.callApi('get', `app/get_blogs?page=${page}&total=${this.total}`)
			if(res.status==200){
